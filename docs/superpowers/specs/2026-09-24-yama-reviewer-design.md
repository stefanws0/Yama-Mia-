...
- **Plugin executor:** a single-thread daemon executor created in `startUp` and stopped with `shutdown()` in `shutDown`, never awaited. `shutdown()`, not `shutdownNow()`: `shutDown` first ends a running fight as `LEFT` on the client thread, which queues that kill's raw-log write, and that queued write must still finish; neither call blocks. All file access runs on it. RuneLite's shared executor is not used, because a slow task there would stall every other plugin.
